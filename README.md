...
- Six-avatar 3×2 comparison view with independent per-cell avatar/effect selection, RANDOM 1–59 distribution using six distinct indexed movements, explicit NO.60 1→6 effect mapping, repeat-one-avatar ×6, synchronize-one-effect ×6, and a shared bottom transport for play/pause, timeline scrubbing, 0.05×–50× speed, and restart-all. EDIT MASTER opens Cell 01 in Single View and APPLY SETTINGS ×6 copies its complete camera, trace, avatar style, lighting, and analysis configuration across the grid while preserving each cell's avatar and effect. RESET ALL restores the canonical six-cell configuration at 1× / 24 fps
...
- Real-time skeletal animation playback, scrubbing, always-visible 0.05×–50× speed control calibrated so 1× equals the native 24 fps, orbit, and zoom
- Hidden-by-default Sequence Timeline for movements 1–59. Add the selected movement, edit any sequence slot, remove or reorder slots, jump directly to a movement, or click a transition card to preview the joint-level morph. Every transition card has its own speed and Linear/Ease In-Out controls. Animation clips blend on one shared skeleton without exposing the T-pose, and the last movement blends back into the first for a continuous loop
- Mix-Up combines five independently looping body regions from movements 1–59 and GLB Style 2. Cross-rig sources transfer compatible joint-rotation deltas only, preserving the selected target avatar's root position, bone lengths, scale, and body structure; automatic random methods remain limited to the lighter indexed 1–59 collection
- Sequence state is encoded with the rest of the shareable view state. Grid View plays imported sequences as read-only animation, preserves them when a cell is opened and returned from Single View, and provides a per-cell SEQ ×6 action for applying one choreography to all six avatars without exposing timeline editing inside the grid
- Unified compact 260px left control rail for Avatar Style, Camera, Trace, NO.60 Elements, Flow Field, and Motion Analysis; the entire rail scrolls above the transport bar so long drawers are never cut off, Trace and Controls share one box while Color Dots has its own separate launcher, Flow Field display and controls share one launcher row, and menu choices use a consistent two-column layout
...
- Sidebar Body Position mode switches between the default fixed-center analysis view and visible root motion that lets the dancer travel through the scene
